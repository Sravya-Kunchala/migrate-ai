  ...
  return (
    <section style={{ background: "#0a0d0f", width: "100%" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "72px 48px" }}>
        <h2 className="pioneer-title" style={{ textAlign: "center", fontSize: 28, fontWeight: 700, color: "#fff", margin: "0 0 48px", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
          Meet the Pioneers
        </h2>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "center", gap: 16 }}>
          {team.map((member, i) => (
            <div key={member.name} className={`pioneer-${i} pioneer-card`} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
              <div style={{
              ...
              <div style={{ textAlign: "center" }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: "#fff", margin: "0 0 3px", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{member.name}</p>
                <p style={{ fontSize: 11.5, color: member.featured ? "#00C96B" : "rgba(255,255,255,0.4)", margin: 0, fontFamily: "'DM Sans', sans-serif" }}>{member.role}</p>
              </div>
            </div>
          ))}
        </div>
